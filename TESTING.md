...
The public API and integrations phase was validated live: `school_api_keys`, `api_request_windows`, `webhook_subscriptions`, and `notification_deliveries` all have RLS enabled and forced; the API-key, webhook, public-school, and delivery-queue functions exist; and the `notification_delivery_queue` trigger is attached to `notifications`. The frontend build, all source JavaScript syntax checks, service-worker syntax check, Edge Function structure checks, and `git diff --check` passed. Deno was unavailable in the sandbox, so TypeScript Edge Functions received structural rather than Deno type-check validation. Cloudflare DNS, API Shield/WAF, rate-limit rules, and Edge Function deployment remain production-account operations and were documented but not executed here.

The monitoring/recovery phase was validated live: `platform_health_checks`, `platform_incidents`, `platform_backup_runs`, `platform_restore_drills`, and `platform_reliability_runs` all have forced RLS; `amaedu-reliability-checks` is scheduled at `*/15 * * * *`; and a controlled run completed with `checks_run=2`, `checks_failed=0`, and `status=completed`. The Operations page build, all source JavaScript syntax checks, and `git diff --check` passed. A provider backup and restore were not executed against production: the application records backup/restore evidence, while Supabase Dashboard/PITR, CLI exports, Storage backups, and isolated restore drills require production credentials and downtime planning.

The final release phase added `npm run test:release`, which validates required artifacts, all 74 source JavaScript files, route registration, Edge Function configuration, migration numbering, release migrations, and built-output credential patterns. `npm run build && npm run test:release` passed; anonymous live RPC probes for maintenance, status, and school directory passed; the service-role database release matrix returned no failed forced-RLS or policy checks; and authenticated read-only tenant probes for two school users returned zero cross-school rows across students, staff, classes, subjects, terms, scores, attendance, and notifications. The historical transactional rollback test was moved to `scripts/rollback_tests/` so it cannot be mistaken for a forward production migration. Authenticated browser flows, provider webhooks, payment actions, and destructive restore operations were not executed because they require real user sessions, external secrets, or production downtime/records.
