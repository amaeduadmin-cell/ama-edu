-- AMA EDU 0058 — fix reliability runner output-column ambiguity
...
declare
  ...
  v_count integer := 0;
  v_failed integer := 0;
  v_run public.platform_reliability_runs%rowtype;
...
begin
  ...
  perform public.record_platform_health_check('database','operational',round(extract(epoch from clock_timestamp()-v_started)*1000)::integer,null,'database-cron');
  v_count := v_count + 1;
  if not exists(select 1 from public.platform_service_status where service_key='database') then v_failed := v_failed + 1; end if;
  perform public.record_platform_health_check('billing-automation',case when exists(select 1 from public.platform_automation_runs where job_name='billing_automation' and started_at > now()-interval '26 hours' and status='completed') then 'operational' else 'degraded' end,null,null,'database-cron');
  v_count := v_count + 1;
